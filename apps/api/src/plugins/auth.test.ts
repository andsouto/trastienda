...
import { installAuth, type VerifyToken } from './auth.ts';
...
it('fails loudly when a route outside a protected scope reads the caller', async () => {
  const app = fastify();
  installAuth(app);
  app.get('/leak', request => request.caller().subject);

  const response = await app.inject({ method: 'GET', url: '/leak' });

  expect(response.statusCode).toBe(500);
  expect(response.json()).toMatchObject({ message: '/leak read the caller but is not inside a protected scope' });
