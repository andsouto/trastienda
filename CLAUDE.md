...
- **Frontend**: Angular 22 (standalone, signals, zoneless), Vitest via `ng test`.
  ...
  **orval** (`retrievalClient: 'both'` — `httpResource` for reads, `HttpClient` services
  for writes); interceptors are where auth, retries and the offline cart queue live
  (ADR-0007). UI library narrowed to Taiga UI (leading) vs ng-zorro-antd, decided by a
  spike once there are real screens.
